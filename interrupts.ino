...
void setInterruptDetails(void)
{
  ...
  TIMSK2 = 0;
  TCNT1 = countUp.txWidthCount;                // Load Timer1 with TX-on count
...
ISR(TIMER1_OVF_vect)
{
  switch (intState) {
    ...
    case 1:
      TCNT1 = countUp.sampleDelayCount;                      // Load Timer1 with main sample delay count
      //TCNT1 = 0xfffd;
    ...
    case 2:
      ...
      TCNT1 = countUp.sampleWidthCount;                     // Load Timer1 with main sample pulse count
      break;

    case 3:
      TCNT1 = countUp.efeDelayCount;                       // Load Timer1 with EFE sample delay count
      PORTB |= (1 << 1);                           // Sample pulse disable
      break;

    case 4:
      TCNT1 = countUp.efeWidthCount;                      // Load Timer1 with EFE sample pulse count
      PORTB &= ~(1 << 2);                          // EFE pulse enable
      break;

    case 5:
      TCNT1 = countUp.aquirePeriod;                       // Load Timer1 with TX period count
